...
5. **Handle references, if named.**
   - Path under `assets/generate/refs/`: check it first against the reference-quality gate in `cinematography.md` (resolution, scene match; for identity, prefer several angles of the same face), then run `${CLAUDE_PLUGIN_ROOT}/shared/kie/kie-upload.sh <path>`.
...
9. **Launch `${CLAUDE_PLUGIN_ROOT}/shared/kie/kie-poll.sh <taskId> <run-folder> video <slug>` as a background job.** Report the `taskId` and folder path immediately. Do not block the terminal on it.
